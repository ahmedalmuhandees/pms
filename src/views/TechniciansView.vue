...
import type { Complex, CreateTechnicianDto, Technician } from '@/types'
...
import { asSelectOptions, labelOf, maintenanceCategoryOptions } from '@/utils/enums'
...
} = usePagedList<Technician, TechnicianParams>(getTechnicians)
...
const form = reactive<CreateTechnicianDto>({
  name: '',
  phone: '',
  department: MaintenanceCategory.Electrical,
  complexId: '',
})
...
async function remove(row: Technician) {
  if (!(await ask(`حذف الفني "${row.name || ''}"؟`))) return
  try {
...
<template>
  <div class="page">
    <PageHeader title="الفنيون" subtitle="إدارة فنيي الصيانة حسب التخصص">
      <template #actions>
        <Button label="إضافة فني" icon="pi pi-plus" @click="openCreate" />
      </template>
    </PageHeader>

    <div class="data-panel">
      <FilterBar v-model:search="search" placeholder="ابحث بالاسم أو الهاتف..." @search="onSearch">
        <template #filters>
          <Select
          ...
          <Select
            ...
            show-clear
            @update:model-value="(v: number | null) => setFilter('Department', v ?? undefined)"
          />
        </template>
      </FilterBar>
      ...
      >
        ...
        <Column field="phone" header="الهاتف" style="width: 140px" />
        <Column header="التخصص" style="width: 140px">
          <template #body="{ data }">{{ labelOf(maintenanceCategoryOptions, data.department) }}</template>
        </Column>
    ...
    <Dialog
      ...
      modal
      :header="editingId ? 'تعديل فني' : 'إضافة فني'"
      :style="{ width: '480px' }"
    >
      <div class="form-grid">
        <div class="field">
