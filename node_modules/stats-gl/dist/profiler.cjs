...
class StatsProfiler extends core.StatsCore {
  constructor(options = {}) {
    super(options);
  ...
  async captureTexture(source, sourceId = "default") {
    if (this.gl) {
      if (!this.textureCaptureWebGL) {
        this.textureCaptureWebGL = new textureCapture.TextureCaptureWebGL(this.gl);
      }
    ...
    if (this.gpuDevice) {
      if (!this.textureCaptureWebGPU) {
        this.textureCaptureWebGPU = new textureCapture.TextureCaptureWebGPU(this.gpuDevice);
      }
...
exports.StatsProfiler = StatsProfiler;
