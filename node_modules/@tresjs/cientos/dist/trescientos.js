...
//#region src/core/abstractions/Align.vue?vue&type=script&setup=true&lang.ts
...
//#endregion
//#region src/core/abstractions/Align.vue
...
//#endregion
//#region src/core/abstractions/CameraShake.vue?vue&type=script&setup=true&lang.ts
...
//#endregion
//#region src/core/abstractions/CameraShake.vue
var CameraShake_default = CameraShake_vue_vue_type_script_setup_true_lang_default;

//#endregion
//#region src/core/abstractions/Decal/DecalGeometry.ts
/**
...
const tmpCorners = Array.from({ length: 8 }, () => new Vector3());
...
*/
function invalidateDecalGeometry(target) {
	cache.delete(target);
}
/**
...
*/
function updateDecalGeometry(target, parent, entry, options = {}) {
	if (!target || !parent) return;
	const flipX = !!entry.flipX;
	const flipY = !!entry.flipY;
	const cached = cache.get(target);
	if (target.geometry && cached && cached.zIndex === entry.zIndex && cached.flipX === flipX && cached.flipY === flipY && cached.position.equals(entry.position) && cached.orientation.equals(entry.orientation) && cached.size.equals(entry.size)) return;
	const { baseOffset = .01, layerGap = .001, cullThreshold = .2 } = options;
	parent.updateWorldMatrix(true, false);
	target.geometry?.dispose();
	const projector = parent.geometry?.boundsTree ? buildBVHCandidateMesh(parent, entry) : parent;
	...
	if (cullThreshold > 0) cullTrianglesByProjectionAngle(geometry, tmpLocalNormal, cullThreshold);
	const parentScale = parent.scale.x || 1;
	const totalOffset = (baseOffset + entry.zIndex * layerGap) / parentScale;
	geometry.translate(tmpLocalNormal.x * totalOffset, tmpLocalNormal.y * totalOffset, tmpLocalNormal.z * totalOffset);
	if (flipX || flipY) {
...
*/
function buildBVHCandidateMesh(parent, entry) {
	const bvh = parent.geometry.boundsTree;
	const src = parent.geometry;
	const srcPos = src.attributes.position;
	if (!srcPos) return parent;
	const srcNorm = src.attributes.normal;
	...
	bvh.shapecast({
		intersectsBounds: (box) => tmpBox.intersectsBox(box),
...
*/
function computeDecalOBBLocalAABB(entry, parentInverseWorld, out) {
	const sx = entry.size.x * .5;
	const sy = entry.size.y * .5;
	const sz = entry.size.z * .5;
	const signs = [-1, 1];
	let k = 0;
	for (const dx of signs) for (const dy of signs) for (const dz of signs) tmpCorners[k++].set(dx * sx, dy * sy, dz * sz);
	out.makeEmpty();
	for (let c = 0; c < 8; c++) {
...
*/
function cullTrianglesByProjectionAngle(geometry, projectorDir, threshold) {
...
function getTextureName(texture$1) {
	if (!texture$1) return null;
	if (texture$1.name) return texture$1.name;
	const userDataName = texture$1.userData?.name;
	if (userDataName) return userDataName;
	const src = texture$1.image?.src;
	if (!src) return null;
	try {
...
function ensureTextureNames(textures) {
...
//#endregion
//#region src/core/abstractions/Decal/useDecalEditor.ts
...
function createSession() {
	const editingEntry = shallowRef(null);
	const editingMode = ref(null);
	const lockedMeshUuid = ref(null);
	const hoveredIntersection = shallowRef(null);
	const hoveredMeshUuid = ref(null);
	const cursorNdc = reactive({
		x: 0,
		y: 0
	});
	const scale = ref(1);
	const angle = ref(0);
	const screenAnchor = reactive({
		x: 0,
		y: 0
	});
	const textureIndex = ref(0);
	const textureCount = ref(0);
	const nextZIndexByMesh = reactive({});
	const isUiDragging = ref(false);
	const isCursorOnTarget = ref(false);
	const dragTextureIndex = ref(null);
	const hoveredEntry = shallowRef(null);
	const baseOrientation = new Euler();
	const enteredEntries = /* @__PURE__ */ new Set();
	const proposals = /* @__PURE__ */ new Map();
	let backup = null;
	const entryActionsRegistry = /* @__PURE__ */ new Map();
	const dataSetterRegistry = /* @__PURE__ */ new Map();
	const dataSetterByMeshName = /* @__PURE__ */ new Map();
	const undoStack = shallowRef([]);
	const redoStack = shallowRef([]);
	const commitListeners = /* @__PURE__ */ new Set();
	const deleteListeners = /* @__PURE__ */ new Set();
	...
	function recomputeHovered() {
		let best = null;
		for (const e of enteredEntries) if (!best || e.zIndex > best.zIndex) best = e;
	...
	function beginCreate(meshUuid, init) {
	...
	function beginEdit(meshUuid, entry) {
		if (editingEntry.value === entry) return;
		if (editingEntry.value) if (editingMode.value === "update") commit();
		else cancel();
		if (!canActOn(meshUuid)) return;
		lockedMeshUuid.value = meshUuid;
	...
	function transferCreate(targetMeshUuid, init) {
	...
	function cancel() {
		if (editingMode.value === "update" && editingEntry.value && backup) restoreBackup(editingEntry.value);
	...
	function setHovered(meshUuid, intersection) {
		if (!(dragTextureIndex.value !== null) && !canActOn(meshUuid)) return;
		if (intersection) proposals.set(meshUuid, intersection);
		else proposals.delete(meshUuid);
		resolveHoverFromProposals();
	}
	function clearHovered(meshUuid) {
	...
	function setScreenAnchor(x, y) {
		screenAnchor.x = x;
		screenAnchor.y = y;
	}
	function setNextZIndex(meshUuid, n) {
		nextZIndexByMesh[meshUuid] = Math.max(0, n);
	}
	function getNextZIndex(meshUuid) {
		return nextZIndexByMesh[meshUuid] ?? 0;
	}
	function setUiDragging(active) {
		isUiDragging.value = active;
	}
	function setCursorOnTarget(on) {
		isCursorOnTarget.value = on;
	}
	function setDragTextureIndex(index) {
	...
	function clearHoveredEntry(entry) {
	...
	function registerDecalEntry(id, actions) {
		entryActionsRegistry.set(id, actions);
	}
	function unregisterDecalEntry(id) {
		entryActionsRegistry.delete(id);
	}
	function getEntryActions(id, caller) {
	...
	function setZIndexById(id, newZ) {
	...
	function setHoveredById(id, on) {
		const actions = getEntryActions(id, "setHoveredById");
		if (!actions) return false;
		actions.setHovered(on);
		return true;
	}
	const meshNameByUuid = /* @__PURE__ */ new Map();
	function registerDecalData(meshUuid, meshName, setter) {
	...
	function unregisterDecalData(meshUuid) {
	...
	function setMeshData(meshName, entries, opts) {
		...
		if (editing && editingMeshName === meshName) entries = entries.map((e) => {
			...
			return {
				...
				orientation: [
					editing.orientation.x,
					editing.orientation.y,
					editing.orientation.z
				],
				size: [
					editing.size.x,
					editing.size.y,
	...
	function recordHistory(meshUuid, before, after) {
		if (before === after) return;
		const next = undoStack.value.concat({
			uuid: meshUuid,
			before,
			after
		});
...
function useDecalEditor() {
	const ctx = useTresContext();
	const session = sessions.get(ctx);
	if (!session) throw new Error("[cientos] useDecalEditor() must be called inside a <TresCanvas> after a <Decal> has been mounted.");
	return session;
}
...
const _hoisted_3$9 = ["color"];
var DecalItem_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	__name: "DecalItem",
	props: {
		parent: {
			type: Object,
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		*/
		...
		const edgesGeometry = shallowRef(null);
		...
		const noRaycast = () => {};
		watch(edgesRef, (line$1) => {
			if (line$1) line$1.raycast = noRaycast;
		}, { immediate: true });
		function rebuildEdges(source) {
		...
		function syncMaterial(entry) {
			...
			const stash = material.userData.decalOriginal ?? (material.userData.decalOriginal = {
			...
			if (material.color) {
				...
				if (target !== stash.appliedColor) {
					...
					else if (stash.colorHex !== void 0) material.color.setHex(stash.colorHex);
			...
			if (targetOpacity !== stash.appliedOpacity) {
				if (targetOpacity !== null) {
					...
					if (stash.opacity !== void 0) material.opacity = stash.opacity;
					if (stash.transparent !== void 0) material.transparent = stash.transparent;
				}
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				ref_key: "meshRef",
				ref: meshRef,
				name: `decal-${__props.entry.id}`,
				"render-order": __props.entry.zIndex,
				"material-transparent": true,
				"material-polygonOffset": true,
				"material-polygonOffsetFactor": -(10 + __props.entry.zIndex),
				"material-depthTest": true,
				onClick,
				onPointerenter: onEnter,
				onPointerleave: onLeave
			}, [renderSlot(_ctx.$slots, "default", {}, () => [_cache[0] || (_cache[0] = createElementVNode("TresMeshBasicMaterial", null, null, -1))]), edgesGeometry.value ? (openBlock(), createElementBlock("TresLineSegments", {
				key: 0,
				ref_key: "edgesRef",
				ref: edgesRef,
				geometry: edgesGeometry.value,
				visible: showEdges.value,
				name: `decal-${__props.entry.id}-edges`
			}, [createElementVNode("TresLineBasicMaterial", {
...
//#endregion
//#region src/core/abstractions/Decal/DecalItem.vue
var DecalItem_default = DecalItem_vue_vue_type_script_setup_true_lang_default;
	...
	setup(__props) {
		...
		*/
		...
		function projectToScreen(point) {
			...
			const sx = rect.left + (tmpProject.x * .5 + .5) * rect.width;
			const sy = rect.top + -(tmpProject.y * .5 - .5) * rect.height;
		...
		function updateBaseOrientation(hit) {
			if (!hit.face) return;
			tmpNormal$1.copy(hit.face.normal).transformDirection(props.parent.matrixWorld).normalize();
			orientationDummy.position.copy(hit.point);
			orientationDummy.lookAt(tmpNormal$1.add(hit.point));
			session.baseOrientation.copy(orientationDummy.rotation);
		}
		function applyOrientation(entry) {
		...
		function fillAspectSize(tex, out) {
		...
		function updateHoverLine(hit) {
			const line$1 = lineRef.value;
			if (!line$1) return;
			...
			tmpNormal$1.copy(hit.face.normal).transformDirection(props.parent.matrixWorld).normalize();
			tmpWorldEnd.copy(hit.point).add(tmpNormal$1);
			const lineParent = line$1.parent;
			if (lineParent) {
				tmpInverse$1.copy(lineParent.matrixWorld).invert();
				tmpLocalA.copy(hit.point).applyMatrix4(tmpInverse$1);
				tmpLocalB.copy(tmpWorldEnd).applyMatrix4(tmpInverse$1);
			} else {
				tmpLocalA.copy(hit.point);
				tmpLocalB.copy(tmpWorldEnd);
			}
			const positions = line$1.geometry.attributes.position;
			if (!positions) line$1.geometry.setFromPoints([tmpLocalA.clone(), tmpLocalB.clone()]);
			else {
		...
		watch(lineRef, (line$1) => {
			if (line$1) line$1.geometry.setFromPoints([new Vector3(), new Vector3()]);
		}, { immediate: true });
		watch(() => session.hoveredMeshUuid.value, () => {
			if (session.hoveredMeshUuid.value === meshUuid.value) updateHoverLine(session.hoveredIntersection.value);
			else updateHoverLine(null);
		}, { flush: "sync" });
		let lastCursorX = NaN;
		let lastCursorY = NaN;
		let lastCanAct = false;
		let lastEditingEntry = null;
		const { onBeforeRender } = useLoop();
		onBeforeRender(() => {
			...
			if (canAct === lastCanAct && entry === lastEditingEntry && cx === lastCursorX && cy === lastCursorY) return;
			lastCanAct = canAct;
		...
		useEventListener(canvasTarget, "pointerdown", (event) => {
			isDragging = false;
		...
		useEventListener(canvasTarget, "pointermove", (event) => {
			const ndc = ndcFromEvent(event);
			if (!ndc) return;
		...
		useEventListener(canvasTarget, "dragover", (event) => {
		...
		useEventListener(canvasTarget, "drop", (event) => {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock(Fragment, null, [createElementVNode("TresLine", {
				ref_key: "lineRef",
				ref: lineRef,
				visible: showHoverLine.value,
				name: "cientos-decal-hover-line"
			}, [..._cache[0] || (_cache[0] = [createElementVNode("TresBufferGeometry", null, null, -1), createElementVNode("TresLineBasicMaterial", {
				color: "#0000ff",
				"depth-test": false,
				transparent: true
...
//#endregion
//#region src/core/abstractions/Decal/DecalEditor.vue
var DecalEditor_default = DecalEditor_vue_vue_type_script_setup_true_lang_default;

//#endregion
//#region src/core/abstractions/Decal/Decal.vue?vue&type=script&setup=true&lang.ts
var Decal_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	__name: "Decal",
	props: {
		data: {
			type: Array,
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		*/
		...
		const textures = computed(() => {
			if (!props.map) return [];
			return (Array.isArray(props.map) ? props.map : [props.map]).filter((t) => t != null);
		});
		...
		function resolveTexture(name) {
			...
			if (!found) console.warn(`[cientos] <Decal>: texture "${name}" not found in \`map\`; falling back to the first available.`);
			return found ?? list[0] ?? null;
		}
		const resolvedMesh = computed(() => {
			const m = props.mesh;
			if (!m) return null;
		...
		const parent = computed(() => {
			...
			return anchorParent && anchorParent.isMesh ? anchorParent : null;
		});
		watch(() => props.editable, (editable) => {
			if (editable) console.warn("[cientos] <Decal>: `editable` is enabled — the interactive editor is for authoring decal layouts, NOT for production. Remove the `editable` prop (and unmount `<DecalDebugUI>`) before shipping; persist the resulting `v-model:data` JSON instead.");
		}, { immediate: true });
		const warnedParents = /* @__PURE__ */ new WeakSet();
		watch(parent, (m) => {
			if (!m || warnedParents.has(m)) return;
			warnedParents.add(m);
			if (m.isSkinnedMesh) console.warn("[cientos] <Decal>: parent is a SkinnedMesh — projected geometry is baked at build time and will NOT deform with the skeleton.");
			const morphs = m.geometry?.morphAttributes;
			if (morphs && Object.keys(morphs).length > 0) console.warn("[cientos] <Decal>: parent has morph targets — projected geometry is baked at build time and will NOT follow morphing.");
			const s = m.scale;
			if (s && (Math.abs(s.x - s.y) > 1e-4 || Math.abs(s.x - s.z) > 1e-4)) console.warn(`[cientos] <Decal>: parent has non-uniform scale (${s.x.toFixed(3)}, ${s.y.toFixed(3)}, ${s.z.toFixed(3)}). Polygon-offset uses scale.x as a single divisor — z-layering may drift on the other axes. Consider uniform scaling.`);
		}, { immediate: true });
		const stampedEntries = shallowRef([]);
		function rebuildEntries() {
			stampedEntries.value = props.data.map((entry) => ({
				id: entry.id,
		...
		const renderedEntries = computed(() => {
			...
			if (session.editingMode.value === "create" && editing && m && session.lockedMeshUuid.value === m.uuid) return [...stampedEntries.value, editing];
		...
		function serialize(entry) {
			...
			if (entry.opacity !== null && entry.opacity !== void 0) json.opacity = entry.opacity;
			if (entry.visible === false) json.visible = false;
			return json;
		}
		function isForThisDecal() {
			const m = parent.value;
			return !!m && session.lockedMeshUuid.value === m.uuid;
		}
		function recordHistoryFor(before, after) {
			const m = parent.value;
		...
		const offCommit = session.onCommit((entry, mode) => {
			...
			const after = mode === "create" ? [...before, json] : before.map((d) => d.id === entry.id ? json : d);
		...
		function projectWorldToScreen(world) {
			...
			const rect = canvas.getBoundingClientRect();
			const sx = rect.left + (tmpProj.x * .5 + .5) * rect.width;
			const sy = rect.top + -(tmpProj.y * .5 - .5) * rect.height;
			session.setScreenAnchor(sx, sy);
		}
		function computeBaseDecalWidth(map) {
			return getTextureAspect(map).x * props.baseSize;
		}
		function seedSessionFromEntry(entry, m) {
			...
			if (!hit?.face) return;
			tmpNormal$1.copy(hit.face.normal).transformDirection(m.matrixWorld).normalize();
			tmpDummy.position.copy(hit.point);
			tmpDummy.lookAt(tmpNormal$1.add(hit.point));
			session.baseOrientation.copy(tmpDummy.rotation);
			session.setAngle(entry.orientation.z - tmpDummy.rotation.z);
		}
		function beginEditForEntry(entry) {
			const m = parent.value;
			if (!m) {
		...
		function setZIndexForId(id, newZ) {
		...
		function setVisibilityForId(id, visible) {
			const i = props.data.findIndex((d) => d.id === id);
			if (i === -1) return;
			if (props.data[i].visible !== false === visible) return;
			const before = props.data;
			const next = before.slice();
		...
		function removeForId(id) {
			if (props.data.findIndex((d) => d.id === id) === -1) return;
			const before = props.data;
		...
		watch([parent, () => parent.value?.name], ([m, name], prev) => {
			...
			if (m) session.registerDecalData(m.uuid, name ?? null, (next, opts) => {
				const before = props.data;
				emit("update:data", next);
		...
		watch(stampedEntries, (entries) => {
			...
			for (const id of registeredIds) if (!nextIds.has(id)) {
		...
		onBeforeUnmount(() => {
			for (const id of registeredIds) session.unregisterDecalEntry(id);
			registeredIds.clear();
			const m = parent.value;
		...
		__expose({
			editor: session,
			beginEditById: (id) => session.beginEditById(id),
			commit: () => session.commit(),
			cancel: () => session.cancel(),
			remove: () => session.remove()
		});
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock(Fragment, null, [createElementVNode("TresGroup", {
				ref_key: "anchorRef",
				ref: anchorRef,
				visible: false,
				name: "cientos-decal-anchor"
			}, null, 512), parent.value ? (openBlock(), createElementBlock(Fragment, { key: 0 }, [(openBlock(true), createElementBlock(Fragment, null, renderList(renderedEntries.value, (entry) => {
				return openBlock(), createBlock(DecalItem_default, {
					key: entry.id,
					parent: parent.value,
					entry,
					editable: __props.editable,
					"base-offset": __props.baseOffset,
					"layer-gap": __props.layerGap,
					"cull-threshold": __props.cullThreshold,
					"edge-color": __props.edgeColor,
					onClick: onItemClick
				}, {
			...
			}), 128)), __props.editable ? (openBlock(), createBlock(DecalEditor_default, {
				key: 0,
				parent: parent.value,
				textures: textures.value,
				"base-size": __props.baseSize
			}, null, 8, [
				"parent",
				"textures",
				"base-size"
			])) : createCommentVNode("v-if", true)], 64)) : createCommentVNode("v-if", true)], 64);
		};
	}
});

//#endregion
//#region src/core/abstractions/Decal/Decal.vue
var Decal_default = Decal_vue_vue_type_script_setup_true_lang_default;

//#endregion
//#region src/core/abstractions/Decal/DebugUI/context.ts
/** Internal — triggers a browser download for a `DecalLayout` JSON. */
function downloadDecalLayout(layout, filename) {
...
const DECAL_DEBUG_KEY = Symbol("cientos-decal-debug-ui");
...
const _hoisted_3$8 = ["transform"];
const _hoisted_4$6 = ["y2", "stroke-width"];
const _hoisted_5$3 = {
	key: 0,
	class: "live-badge"
};
const _hoisted_6$2 = { class: "live-badge__scale" };
const _hoisted_7$2 = { class: "live-badge__layer" };
const MIN_SCALE = .25;
const MAX_SCALE = 3.5;
const FADE_OUT_MS = 220;
var Handle_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	__name: "Handle",
	setup(__props) {
		/**
		...
		*/
		...
		const { x: xDrag, y: yDrag } = useDraggable(dragRef, {
			...
			onStart(_position, event) {
				if (!isEditing.value) return false;
				if (sliderRef.value?.contains(event.target)) return false;
		...
		const innerStyle = computed(() => ({ transform: `scale(${currentScale.value})` }));
		watch([xDrag, yDrag], () => {
		...
		function updateSliderFromAngle(angle) {
		...
		watch(computed(() => session.value?.editingEntry.value ?? null), async (entry) => {
		...
		watch(isDraggingTexture, async (now, prev) => {
			if (prev && !now && session.value?.editingEntry.value) await snapToCurrentEntry();
		});
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", { class: normalizeClass(["handle-wrapper", isVisible.value ? "is-visible" : ""]) }, [createElementVNode("div", {
				ref_key: "dragRef",
				ref: dragRef,
				style: normalizeStyle(containerStyle.value),
				class: normalizeClass(["handle-area", isVisible.value ? "is-active" : ""])
				...
				withDirectives((openBlock(), createElementBlock("svg", _hoisted_2$30, [(openBlock(true), createElementBlock(Fragment, null, renderList(snapTicks.value, (angle, i) => {
					...
					}, null, 8, _hoisted_4$6)], 8, _hoisted_3$8);
				}), 128))], 512)), [[vShow, unref(isSnapEnabled)]]),
				_cache[0] || (_cache[0] = createElementVNode("div", { class: "circle-center" }, null, -1)),
				_cache[1] || (_cache[1] = createElementVNode("div", { class: "circle-ring" }, null, -1)),
				createElementVNode("div", {
					ref_key: "sliderRef",
					ref: sliderRef,
					class: "slider-anchor"
				}, [createElementVNode("div", { class: normalizeClass(["slider-handle", { "is-dragging": isSliderDragging.value }]) }, null, 2)], 512)
			], 4), isEditing.value ? (openBlock(), createElementBlock("div", _hoisted_5$3, [
				createElementVNode("span", _hoisted_6$2, toDisplayString(scalePercent.value), 1),
				_cache[3] || (_cache[3] = createElementVNode("span", { class: "live-badge__sep" }, "·", -1)),
				createElementVNode("span", null, toDisplayString(rotationDeg.value), 1),
				editingZIndex.value !== null ? (openBlock(), createElementBlock(Fragment, { key: 0 }, [_cache[2] || (_cache[2] = createElementVNode("span", { class: "live-badge__sep" }, "·", -1)), createElementVNode("span", _hoisted_7$2, "L" + toDisplayString(editingZIndex.value), 1)], 64)) : createCommentVNode("v-if", true)
			])) : createCommentVNode("v-if", true)], 6)], 2);
		};
	}
});

//#endregion
//#region src/core/abstractions/Decal/DebugUI/Handle.vue
var Handle_default = Handle_vue_vue_type_script_setup_true_lang_default;
...
const _hoisted_3$7 = { class: "tint-row" };
const _hoisted_4$5 = ["value"];
const _hoisted_5$2 = { class: "tint-value" };
const _hoisted_6$1 = { class: "tint-row" };
const _hoisted_7$1 = ["value"];
const _hoisted_8$1 = { class: "tint-value" };
const _hoisted_9$1 = ["title"];
const _hoisted_10$1 = { class: "layer-val tabular-nums" };
const _hoisted_11$1 = { class: "dock dock--persistent" };
const _hoisted_12$1 = {
	key: 1,
	class: "mode-badge mode-badge--idle"
};
const _hoisted_13$1 = {
	key: 2,
	class: "picker"
...
const _hoisted_15$1 = { class: "thumb__frame" };
...
const _hoisted_18 = { class: "export-slot" };
...
var Dock_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		*/
		...
		const tintWrapperRef = useTemplateRef("tintWrapperRef");
		...
		function toggleTint() {
			if (!isTintOpen.value) {
				const s = session.value;
				if (s && s.editingEntry.value && s.editingEntry.value.color === null) s.setColor(colorInputValue.value);
			}
			isTintOpen.value = !isTintOpen.value;
		}
		function onColorChange(event) {
			session.value?.setColor(event.target.value);
		}
		function onOpacityChange(event) {
			session.value?.setOpacity(Number(event.target.value));
		}
		function resetTint() {
		...
		function onDocClick(event) {
			...
			if (root && !root.contains(event.target)) isTintOpen.value = false;
		}
		...
		function selectTexture(i) {
			session.value?.setTextureIndex(i);
		}
		function toggleSnap() {
			isSnapEnabled.value = !isSnapEnabled.value;
		}
		function toggleFlipX() {
			session.value?.toggleFlipX();
		}
		function toggleFlipY() {
			session.value?.toggleFlipY();
		}
		function changeZIndex(delta) {
			session.value?.changeZIndex(delta);
		}
		function onThumbDragStart(event, i) {
			const s = session.value;
			if (!s) return;
		...
		function onThumbDragEnd(event) {
			const s = session.value;
			if (!s) return;
			if (event.dataTransfer?.dropEffect === "none" && s.editingMode.value === "create") s.cancel();
				...
				createVNode(Transition, { name: "edit-dock" }, {
					default: withCtx(() => [isEditing.value && !isDraggingTexture.value ? (openBlock(), createElementBlock("div", _hoisted_1$60, [
						createElementVNode("button", {
							...
							createElementVNode("path", { d: "M6 15a6 6 0 0 0 12 0V3h-4v12a2 2 0 0 1-4 0V3H6Z" }),
							createElementVNode("line", {
								x1: "6",
								y1: "8",
								x2: "10",
						...
						createElementVNode("button", {
							class: normalizeClass(["icon-btn", { "is-active": editingFlipX.value }]),
							...
							createElementVNode("path", { d: "M16 7l4 5-4 5" }),
							createElementVNode("path", { d: "M8 7L4 12l4 5" })
						], -1)])], 2),
						createElementVNode("button", {
							class: normalizeClass(["icon-btn", { "is-active": editingFlipY.value }]),
							title: "Flip vertical",
							onClick: toggleFlipY
							...
							createElementVNode("path", { d: "M3 12h18" }),
							createElementVNode("path", { d: "M7 8l5-4 5 4" }),
							createElementVNode("path", { d: "M7 16l5 4 5-4" })
						], -1)])], 2),
						createElementVNode("div", {
							ref_key: "tintWrapperRef",
							ref: tintWrapperRef,
							class: "tint-wrapper"
						}, [createElementVNode("button", {
							class: normalizeClass(["icon-btn", { "is-active": hasCustomTint.value || isTintOpen.value }]),
							title: "Tint & opacity",
							onClick: toggleTint
							...
							"stroke-linecap": "round",
							"stroke-linejoin": "round"
						}, [createElementVNode("path", { d: "M9 4v14a3 3 0 1 0 6 0V4a1 1 0 0 0-1-1h-4a1 1 0 0 0-1 1Z" }), createElementVNode("path", { d: "M9 9h6" })], -1)), editingColor.value ? (openBlock(), createElementBlock("span", {
							key: 0,
							class: "tint-swatch",
							style: normalizeStyle({ backgroundColor: editingColor.value })
						}, null, 4)) : createCommentVNode("v-if", true)], 2), isTintOpen.value ? (openBlock(), createElementBlock("div", _hoisted_2$29, [
							createElementVNode("div", _hoisted_3$7, [
								_cache[10] || (_cache[10] = createElementVNode("span", { class: "tint-row__label" }, "Color", -1)),
								...
								createElementVNode("span", _hoisted_5$2, toDisplayString(colorInputValue.value), 1)
							]),
							createElementVNode("div", _hoisted_6$1, [
								_cache[11] || (_cache[11] = createElementVNode("span", { class: "tint-row__label" }, "Opacity", -1)),
								createElementVNode("input", {
									class: "tint-range",
									type: "range",
									min: "0",
									max: "1",
									step: "0.05",
									value: opacityInputValue.value,
									onInput: onOpacityChange
								}, null, 40, _hoisted_7$1),
								createElementVNode("span", _hoisted_8$1, toDisplayString(opacityDisplay.value), 1)
							]),
							createElementVNode("button", {
								class: "tint-reset",
								type: "button",
						...
						_cache[18] || (_cache[18] = createElementVNode("span", { class: "divider" }, null, -1)),
						createElementVNode("div", {
							class: "layer-control",
							title: `Layer ${editingZIndex.value ?? 0}`
						}, [
							createElementVNode("button", {
								class: "layer-btn",
								title: "Send back",
								onClick: _cache[0] || (_cache[0] = ($event) => changeZIndex(-1))
							...
							createElementVNode("span", _hoisted_10$1, "L" + toDisplayString(editingZIndex.value ?? 0), 1),
							createElementVNode("button", {
								class: "layer-btn",
								title: "Bring forward",
								onClick: _cache[1] || (_cache[1] = ($event) => changeZIndex(1))
						...
						isUpdating.value ? (openBlock(), createElementBlock(Fragment, { key: 0 }, [_cache[15] || (_cache[15] = createElementVNode("span", { class: "divider" }, null, -1)), createElementVNode("button", {
							class: "icon-btn is-danger",
							title: "Delete (Del)",
							onClick: _cache[2] || (_cache[2] = (...args) => unref(remove) && unref(remove)(...args))
						...
						], -1)])])], 64)) : createCommentVNode("v-if", true),
						_cache[19] || (_cache[19] = createElementVNode("span", { class: "divider" }, null, -1)),
						createElementVNode("button", {
							class: "icon-btn is-danger",
							title: "Cancel (Esc)",
							onClick: _cache[3] || (_cache[3] = (...args) => unref(cancel) && unref(cancel)(...args))
						...
						createElementVNode("button", {
							class: "primary-btn",
							title: "Confirm (Enter)",
							onClick: _cache[4] || (_cache[4] = (...args) => unref(commit) && unref(commit)(...args))
						}, [..._cache[17] || (_cache[17] = [
							...
							createElementVNode("span", null, "Confirm", -1),
							createElementVNode("kbd", { class: "kbd" }, "⏎", -1)
						])])
					])) : createCommentVNode("v-if", true)]),
					_: 1
				}),
				createCommentVNode(" Bottom row: persistent palette (visible whenever textures exist). "),
				createElementVNode("div", _hoisted_11$1, [
					isEditing.value || isDraggingTexture.value ? (openBlock(), createElementBlock("div", {
						key: 0,
						class: normalizeClass(["mode-badge", isUpdating.value ? "is-update" : "is-place"])
					}, [_cache[20] || (_cache[20] = createElementVNode("span", { class: "mode-badge__dot" }, [createElementVNode("span", { class: "mode-badge__ping" }), createElementVNode("span", { class: "mode-badge__core" })], -1)), createTextVNode(" " + toDisplayString(isUpdating.value ? "Editing" : isEditing.value ? "Placing" : "Drop to place"), 1)], 2)) : (openBlock(), createElementBlock("div", _hoisted_12$1, [..._cache[21] || (_cache[21] = [createElementVNode("span", { class: "mode-badge__core" }, null, -1), createTextVNode(" Pick a logo ", -1)])])),
					unref(textures).length ? (openBlock(), createElementBlock("div", _hoisted_13$1, [(openBlock(true), createElementBlock(Fragment, null, renderList(unref(textures), (tex, i) => {
						return openBlock(), createElementBlock("button", {
							key: tex.uuid || i,
							title: `${unref(getTextureLabel)(tex)} — drag onto the model or click then click the model`,
							draggable: true,
							class: normalizeClass(["thumb", [i === textureIndex.value ? "is-active" : "", i === dragTextureIndex.value ? "is-dragging" : ""]]),
							onClick: ($event) => selectTexture(i),
							onDragstart: (e) => onThumbDragStart(e, i),
							onDragend: onThumbDragEnd
						}, [createElementVNode("span", _hoisted_15$1, [unref(getTextureSrc)(tex) ? (openBlock(), createElementBlock("img", {
							key: 0,
							src: unref(getTextureSrc)(tex),
							alt: unref(getTextureLabel)(tex),
							class: "thumb__img",
							draggable: "false"
						}, null, 8, _hoisted_16$1)) : createCommentVNode("v-if", true)]), i === textureIndex.value ? (openBlock(), createElementBlock("span", _hoisted_17, [..._cache[22] || (_cache[22] = [createElementVNode("svg", {
							width: "10",
							height: "10",
							viewBox: "0 0 24 24",
							fill: "none",
							...
							"stroke-linecap": "round",
							"stroke-linejoin": "round"
						}, [createElementVNode("polyline", { points: "20 6 9 17 4 12" })], -1)])])) : createCommentVNode("v-if", true)], 42, _hoisted_14$1);
					...
					createElementVNode("div", _hoisted_18, [
						createElementVNode("button", {
							...
							"stroke-linecap": "round",
							"stroke-linejoin": "round"
						}, [createElementVNode("path", { d: "M3 7v6h6" }), createElementVNode("path", { d: "M21 17a9 9 0 0 0-15-6.7L3 13" })], -1)])], 8, _hoisted_19),
						createElementVNode("button", {
							class: "icon-btn",
							...
							"stroke-linecap": "round",
							"stroke-linejoin": "round"
						}, [createElementVNode("path", { d: "M21 7v6h-6" }), createElementVNode("path", { d: "M3 17a9 9 0 0 1 15-6.7L21 13" })], -1)])], 8, _hoisted_20),
						_cache[27] || (_cache[27] = createElementVNode("span", { class: "divider" }, null, -1)),
						createElementVNode("button", {
							class: "icon-btn",
							...
							createElementVNode("polyline", { points: "17 8 12 3 7 8" }),
							createElementVNode("line", {
								x1: "12",
								y1: "3",
								x2: "12",
						...
						createElementVNode("button", {
							class: "icon-btn",
							disabled: !unref(hasData),
							title: "Export layout as JSON",
							onClick: _cache[5] || (_cache[5] = (...args) => unref(exportLayout) && unref(exportLayout)(...args))
...
//#endregion
//#region src/core/abstractions/Decal/DebugUI/Dock.vue
...
const _hoisted_5$1 = { class: "list-group__header" };
const _hoisted_6 = { class: "list-group__name" };
const _hoisted_7 = { class: "list-group__count" };
const _hoisted_8 = { class: "list-group__items" };
const _hoisted_9 = [
	"title",
	"onClick",
	"onMouseenter",
...
const _hoisted_13 = { class: "list-item__thumb" };
const _hoisted_14 = ["src", "alt"];
const _hoisted_15 = { class: "list-item__layer" };
const _hoisted_16 = ["onClick"];
var LayerList_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	__name: "LayerList",
	setup(__props) {
		/**
		...
		*/
		...
		const liveLayout = computed(() => {
			...
			const out = {};
			for (const [meshName, entries] of Object.entries(layout.value)) out[meshName] = entries.map((e) => {
				if (e.id !== editing.id) return e;
				return {
					...e,
					zIndex: editing.zIndex,
					flipX: editing.flipX || void 0,
					flipY: editing.flipY || void 0,
					map: editing.map?.name ?? e.map,
					visible: editing.visible === false ? false : void 0
				};
			});
			return out;
		});
		...
		function hoverById(id, on) {
			session.value?.setHoveredById(id, on);
		}
		function toggleVisibility(event, entry) {
			event.stopPropagation();
			if (!session.value) return;
		...
		function onDragStart(event, entry, meshName) {
		...
		function onDragOver(event, entry, meshName) {
		...
		function onDrop(event, target, meshName) {
			...
			return openBlock(), createElementBlock("div", _hoisted_1$59, [
				...
				(openBlock(true), createElementBlock(Fragment, null, renderList(groups.value, (group) => {
					return openBlock(), createElementBlock("section", {
						key: group.meshName,
						class: "list-group"
					}, [createElementVNode("header", _hoisted_5$1, [createElementVNode("span", _hoisted_6, toDisplayString(group.meshName), 1), createElementVNode("span", _hoisted_7, toDisplayString(group.entries.length), 1)]), createElementVNode("div", _hoisted_8, [(openBlock(true), createElementBlock(Fragment, null, renderList(group.entries, (entry) => {
						return openBlock(), createElementBlock("div", {
							key: entry.id,
							title: `${group.meshName} · Layer ${entry.zIndex} — drag to reorder`,
							class: normalizeClass(["list-item", [
								{ "is-editing": editingId.value === entry.id },
								{ "is-dragging": draggedId.value === entry.id },
								{ "is-hidden": entry.visible === false },
								dragOverId.value === entry.id && dragOverPosition.value === "before" ? "drop-before" : "",
								dragOverId.value === entry.id && dragOverPosition.value === "after" ? "drop-after" : ""
							]]),
							draggable: true,
							onClick: ($event) => editById(entry.id),
							onMouseenter: ($event) => hoverById(entry.id, true),
							onMouseleave: ($event) => hoverById(entry.id, false),
							onDragstart: (e) => onDragStart(e, entry, group.meshName),
							onDragover: (e) => onDragOver(e, entry, group.meshName),
							onDragleave: ($event) => onDragLeave(entry),
							onDrop: (e) => onDrop(e, entry, group.meshName),
							onDragend: onDragEnd
						}, [
							createElementVNode("button", {
								type: "button",
								class: normalizeClass(["list-item__eye", { "is-off": entry.visible === false }]),
								...
								draggable: "false",
								onClick: ($event) => toggleVisibility($event, entry),
								onDragstart: _cache[0] || (_cache[0] = withModifiers(() => {}, ["stop", "prevent"]))
							}, [entry.visible !== false ? (openBlock(), createElementBlock("svg", _hoisted_11, [..._cache[3] || (_cache[3] = [createElementVNode("path", {
								fill: "currentColor",
								d: "M8 3.5c-3.2 0-5.8 2.3-7 4.5 1.2 2.2 3.8 4.5 7 4.5s5.8-2.3 7-4.5c-1.2-2.2-3.8-4.5-7-4.5Zm0 7.3a2.8 2.8 0 1 1 0-5.6 2.8 2.8 0 0 1 0 5.6Z"
							}, null, -1), createElementVNode("circle", {
								cx: "8",
								cy: "8",
								r: "1.4",
								fill: "#0b1320"
							}, null, -1)])])) : (openBlock(), createElementBlock("svg", _hoisted_12, [..._cache[4] || (_cache[4] = [createElementVNode("path", {
								fill: "currentColor",
								d: "M2.7 2 1.6 3.1l2.1 2.1A11 11 0 0 0 1 8c1.2 2.2 3.8 4.5 7 4.5 1.1 0 2.1-.3 3-.7l2 2 1-1L2.7 2Zm5.3 3.5c.6 0 1.2.1 1.7.4l-.9.9a1.4 1.4 0 0 0-1.7 1.7l-.9.9A2.8 2.8 0 0 1 8 5.5Zm0 5.6c-.4 0-.8-.1-1.2-.2L9 9.2c0 .7-.5 1.3-1 1.5l0 .4Zm5.4-1.6A6.5 6.5 0 0 0 15 8a7.2 7.2 0 0 0-5.9-4.4l-.8.8A6.4 6.4 0 0 1 14 8c-.4.7-1 1.3-1.6 1.8l1 .7Z"
							}, null, -1)])]))], 42, _hoisted_10),
							createElementVNode("span", _hoisted_13, [entry.map && thumbSrcByName.value.get(entry.map) ? (openBlock(), createElementBlock("img", {
								key: 0,
								src: thumbSrcByName.value.get(entry.map),
								alt: entry.map,
								class: "list-item__img",
								draggable: "false"
							}, null, 8, _hoisted_14)) : createCommentVNode("v-if", true)]),
							createElementVNode("span", _hoisted_15, "L" + toDisplayString(entry.zIndex), 1),
							createElementVNode("button", {
								type: "button",
								...
								draggable: "false",
								onClick: ($event) => removeEntry($event, entry),
								onDragstart: _cache[1] || (_cache[1] = withModifiers(() => {}, ["stop", "prevent"]))
...
//#endregion
//#region src/core/abstractions/Decal/DebugUI/LayerList.vue
var LayerList_default = LayerList_vue_vue_type_script_setup_true_lang_default;

//#endregion
//#region src/core/abstractions/Decal/DebugUI/DecalDebugUI.vue?vue&type=script&setup=true&lang.ts
var DecalDebugUI_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	__name: "DecalDebugUI",
	props: {
		session: {
			type: [Object, null],
	...
	setup(__props, { emit: __emit }) {
		...
		*/
		...
		function defaultExportFilename() {
			return `decal-layout-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.json`;
		}
		function exportLayout() {
			if (isEditing.value) session.value?.commit();
			const json = layout.value;
			if (props.exportFilename !== null) downloadDecalLayout(json, props.exportFilename ?? defaultExportFilename());
			emit("export", json);
		}
		/**
		...
		*/
		function sanitiseImported(parsed) {
			...
			const out = {};
			for (const [meshName, raw] of Object.entries(parsed)) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", {
				ref_key: "rootRef",
				ref: rootRef,
				class: normalizeClass(["cientos-decal-ui", [`theme-${__props.theme}`, __props.contained ? "is-contained" : ""]])
			}, [
				createVNode(Handle_default),
				createVNode(Dock_default),
				createVNode(LayerList_default)
			], 2);
		};
	}
});
...
//#endregion
//#region src/core/abstractions/Decal/DebugUI/DecalDebugUI.vue
var DecalDebugUI_default = DecalDebugUI_vue_vue_type_script_setup_true_lang_default;
...
//#endregion
//#region src/core/abstractions/Fit.vue?vue&type=script&setup=true&lang.ts
...
//#endregion
//#region src/core/abstractions/Fit.vue
...
var component_default$11 = component_vue_vue_type_script_setup_true_lang_default$18;
...
var component_default$12 = component_vue_vue_type_script_setup_true_lang_default$17;
...
var component_default$15 = component_vue_vue_type_script_setup_true_lang_default$16;
...
//#endregion
//#region src/core/debug-performance/Helper/useHelper.ts
...
//#endregion
//#region src/core/debug-performance/Helper/component.vue?vue&type=script&setup=true&lang.ts
...
//#endregion
//#region src/core/debug-performance/Helper/component.vue
var component_default$8 = component_vue_vue_type_script_setup_true_lang_default$15;

//#endregion
//#region src/core/debug-performance/LOD.vue?vue&type=script&setup=true&lang.ts
...
//#endregion
//#region src/core/debug-performance/LOD.vue
var LOD_default = LOD_vue_vue_type_script_setup_true_lang_default;

//#endregion
//#region src/core/debug-performance/Stats.ts
...
//#endregion
//#region src/core/debug-performance/StatsGl.ts
...
//#endregion
//#region src/core/debug-performance/useBVH.ts
/**
...
//#endregion
//#region src/core/light-shadow/RandomizedLights/RandomizedLights.ts
...
//#endregion
//#region src/core/light-shadow/RandomizedLights/component.vue
var component_default$14 = component_vue_vue_type_script_setup_true_lang_default$14;
...
//#endregion
//#region src/core/light-shadow/AccumulativeShadows/ProgressiveLightMap.ts
...
//#endregion
//#region src/core/light-shadow/AccumulativeShadows/SoftShadowMaterial.ts
...
//#endregion
//#region src/core/light-shadow/AccumulativeShadows/component.vue
var component_default = component_vue_vue_type_script_setup_true_lang_default$13;

//#endregion
//#region src/core/light-shadow/BakeShadows.ts
...
//#endregion
//#region src/core/light-shadow/CircleShadow.vue
...
//#endregion
//#region src/core/light-shadow/ContactShadows.vue
var ContactShadows_default = ContactShadows_vue_vue_type_script_setup_true_lang_default;

//#endregion
//#region src/core/light-shadow/Lensflare/LensflareImpl.ts
...
//#endregion
//#region src/core/light-shadow/Lensflare/constants.ts
...
//#endregion
//#region src/core/light-shadow/Lensflare/RandUtils.ts
...
//#endregion
//#region src/core/light-shadow/Lensflare/index.ts
...
//#endregion
//#region src/core/light-shadow/Lensflare/component.vue?vue&type=script&setup=true&lang.ts
...
//#endregion
//#region src/core/light-shadow/Lensflare/component.vue
var component_default$10 = component_vue_vue_type_script_setup_true_lang_default$12;

//#endregion
//#region src/core/light-shadow/SoftShadows.vue?vue&type=script&setup=true&lang.ts
const PCSSGetShadow = `
...
//#endregion
//#region src/core/light-shadow/SoftShadows.vue
...
var component_default$17 = component_vue_vue_type_script_setup_true_lang_default$11;
...
var component_default$5 = component_vue_vue_type_script_setup_true_lang_default$10;
...
var component_default$7 = component_vue_vue_type_script_setup_true_lang_default$9;
...
var component_default$18 = component_vue_vue_type_script_setup_true_lang_default$8;
...
var component_vue_vue_type_script_setup_true_lang_default$7 = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose }) {
		...
		__expose({ instance: materialRef });
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("primitive", { object: materialRef.value }, null, 8, _hoisted_1$38);
		};
	}
});
...
var component_default$13 = component_vue_vue_type_script_setup_true_lang_default$7;

//#endregion
//#region src/core/miscellaneous/Bounds/Bounds.ts
...
//#endregion
//#region src/core/miscellaneous/Bounds/component.vue?vue&type=script&setup=true&lang.ts
const _hoisted_1$37 = [
	"object",
	"duration",
	"offset"
...
var component_vue_vue_type_script_setup_true_lang_default$6 = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		return (_ctx, _cache) => {
			...
			}, [renderSlot(_ctx.$slots, "default")], 8, _hoisted_1$37);
		};
	}
});

//#endregion
//#region src/core/miscellaneous/Bounds/component.vue
var component_default$2 = component_vue_vue_type_script_setup_true_lang_default$6;

//#endregion
//#region src/core/miscellaneous/GlobalAudio.ts
...
//#endregion
//#region src/core/miscellaneous/MouseParallax.vue?vue&type=script&setup=true&lang.ts
...
//#endregion
//#region src/core/miscellaneous/MouseParallax.vue
var MouseParallax_default = MouseParallax_vue_vue_type_script_setup_true_lang_default;

//#endregion
//#region src/core/miscellaneous/PositionalAudio.vue?vue&type=script&setup=true&lang.ts
const _hoisted_1$36 = ["args"];
...
//#endregion
//#region src/core/miscellaneous/PositionalAudio.vue
var PositionalAudio_default = PositionalAudio_vue_vue_type_script_setup_true_lang_default;

//#endregion
//#region src/core/miscellaneous/useAnimations.ts
/**
...
//#endregion
//#region src/core/miscellaneous/useGLTFExporter.ts
...
//#endregion
//#region src/core/miscellaneous/useIntersect.ts
...
//#endregion
//#region src/core/objects/AnimatedSprite/AtlasAnimationDefinitionParser.ts
/**
...
//#endregion
//#region src/core/objects/AnimatedSprite/StringOps.ts
...
//#endregion
//#region src/core/objects/AnimatedSprite/Atlas.ts
...
//#endregion
//#region src/core/objects/AnimatedSprite/component.vue?vue&type=script&setup=true&lang.ts
const _hoisted_1$35 = ["scale", "position"];
const _hoisted_2$24 = ["map", "alphaTest"];
...
var component_vue_vue_type_script_setup_true_lang_default$5 = /* @__PURE__ */ defineComponent({
	...
	async setup(__props, { expose: __expose, emit: __emit }) {
		...
		return (_ctx, _cache) => {
			...
			}, null, 8, _hoisted_2$24)], 8, _hoisted_1$35)) : (openBlock(), createElementBlock("TresMesh", {
				key: 1,
				scale: [
					scaleX.value,
					scaleY.value,
...
//#endregion
//#region src/core/objects/AnimatedSprite/component.vue
var component_default$1 = component_vue_vue_type_script_setup_true_lang_default$5;

//#endregion
//#region src/core/objects/CubeCamera/useCubeCamera.ts
...
//#endregion
//#region src/core/objects/CubeCamera/component.vue?vue&type=script&setup=true&lang.ts
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresGroup", {
				ref_key: "groupRef",
				ref: groupRef
			}, [createElementVNode("primitive", { object: unref(camera) }, null, 8, _hoisted_1$34), renderSlot(_ctx.$slots, "default")], 512);
		};
	}
});

//#endregion
//#region src/core/objects/CubeCamera/component.vue
var component_default$3 = component_vue_vue_type_script_setup_true_lang_default$4;

//#endregion
//#region src/core/objects/useFBO/index.ts
...
//#endregion
//#region src/core/objects/useFBO/component.vue?vue&type=script&setup=true&lang.ts
...
//#endregion
//#region src/core/objects/useFBO/component.vue
var component_default$6 = component_vue_vue_type_script_setup_true_lang_default$3;

//#endregion
//#region src/core/objects/GradientTexture.vue?vue&type=script&setup=true&lang.ts
const _hoisted_1$33 = [
	"color-space",
	"args",
	"attach"
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresCanvasTexture", {
				...
				args: [unref(canvas)],
				attach: props.attach
			}, null, 8, _hoisted_1$33);
		};
	}
});

//#endregion
//#region src/core/objects/GradientTexture.vue
...
//#endregion
//#region src/core/objects/html/HTML.vue?vue&type=script&setup=true&lang.ts
const _hoisted_1$32 = [
	"material",
	"cast-shadow",
	"receive-shadow",
...
var HTML_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		return (_ctx, _cache) => {
			...
			}, null, 8, _hoisted_1$32)) : createCommentVNode("v-if", true)], 512);
		};
	}
});

//#endregion
//#region src/core/objects/html/HTML.vue
var HTML_default = HTML_vue_vue_type_script_setup_true_lang_default;

//#endregion
//#region src/core/objects/Image/ImageMaterialImpl.ts
/**
* NOTE: Source:
...
//#endregion
//#region src/core/objects/Image/ImageMaterial.vue?vue&type=script&setup=true&lang.ts
...
//#endregion
//#region src/core/objects/Image/ImageMaterial.vue
var ImageMaterial_default = ImageMaterial_vue_vue_type_script_setup_true_lang_default;

//#endregion
//#region src/core/objects/Image/component.vue?vue&type=script&setup=true&lang.ts
const _hoisted_1$31 = ["scale"];
const _hoisted_2$23 = ["args"];
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				...
				props.segments,
				props.segments
			] }, null, 8, _hoisted_2$23)]), createVNode(ImageMaterial_default, {
				color: props.color,
				map: texture$1.value,
				zoom: props.zoom,
...
//#endregion
//#region src/core/objects/Image/component.vue
var component_default$9 = component_vue_vue_type_script_setup_true_lang_default$2;

//#endregion
//#region src/core/objects/MarchingCubes/const.ts
const MARCHING_CUBES_PROVIDE_KEY = Symbol("marchingCubes");

//#endregion
//#region src/core/objects/MarchingCubes/MarchingCube.vue?vue&type=script&setup=true&lang.ts
...
//#endregion
//#region src/core/objects/MarchingCubes/MarchingCube.vue
var MarchingCube_default = MarchingCube_vue_vue_type_script_setup_true_lang_default;

//#endregion
//#region src/core/objects/MarchingCubes/MarchingCubes.vue?vue&type=script&setup=true&lang.ts
const _hoisted_1$30 = ["object"];
var MarchingCubes_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("primitive", { object: marchingCubes.value }, [renderSlot(_ctx.$slots, "default")], 8, _hoisted_1$30);
		};
	}
});

//#endregion
//#region src/core/objects/MarchingCubes/MarchingCubes.vue
var MarchingCubes_default = MarchingCubes_vue_vue_type_script_setup_true_lang_default;

//#endregion
//#region src/core/objects/MarchingCubes/MarchingPlane.vue?vue&type=script&setup=true&lang.ts
...
//#endregion
//#region src/core/objects/MarchingCubes/MarchingPlane.vue
var MarchingPlane_default = MarchingPlane_vue_vue_type_script_setup_true_lang_default;

//#endregion
//#region src/core/objects/Reflector.vue?vue&type=script&setup=true&lang.ts
const _hoisted_1$29 = ["args", "material-uniforms-color-value"];
var Reflector_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			...
			}, [renderSlot(_ctx.$slots, "default", {}, () => [_cache[0] || (_cache[0] = createElementVNode("TresPlaneGeometry", { args: [5, 5] }, null, -1))])], 8, _hoisted_1$29);
		};
	}
});

//#endregion
//#region src/core/objects/Reflector.vue
var Reflector_default = Reflector_vue_vue_type_script_setup_true_lang_default;

//#endregion
//#region src/core/objects/Text3D.vue?vue&type=script&setup=true&lang.ts
const _hoisted_1$28 = ["args", "center"];
	...
	async setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return unref(font) ? (openBlock(), createElementBlock("TresMesh", {
				...
				center: unref(center)
			}, null, 8, _hoisted_1$28)) : createCommentVNode("v-if", true), renderSlot(_ctx.$slots, "default")], 512)) : createCommentVNode("v-if", true);
		};
	}
});

//#endregion
//#region src/core/objects/Text3D.vue
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				ref_key: "boxRef",
				ref: boxRef
			}, [createElementVNode("TresBoxGeometry", { args: unref(args) }, null, 8, _hoisted_1$27), renderSlot(_ctx.$slots, "default", {}, () => [createElementVNode("TresMeshBasicMaterial", { color: unref(color) }, null, 8, _hoisted_2$22)])], 512);
		};
	}
});
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				ref_key: "circleRef",
				ref: circleRef
			}, [createElementVNode("TresCircleGeometry", { args: unref(args) }, null, 8, _hoisted_1$25), renderSlot(_ctx.$slots, "default", {}, () => [createElementVNode("TresMeshBasicMaterial", { color: unref(color) }, null, 8, _hoisted_2$21)])], 512);
		};
	}
});
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				ref_key: "coneRef",
				ref: coneRef
			}, [createElementVNode("TresConeGeometry", { args: unref(args) }, null, 8, _hoisted_1$24), renderSlot(_ctx.$slots, "default", {}, () => [createElementVNode("TresMeshBasicMaterial", { color: unref(color) }, null, 8, _hoisted_2$20)])], 512);
		};
	}
});
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				ref_key: "cylinderRef",
				ref: cylinderRef
			}, [createElementVNode("TresCylinderGeometry", { args: unref(args) }, null, 8, _hoisted_1$23), renderSlot(_ctx.$slots, "default", {}, () => [createElementVNode("TresMeshBasicMaterial", { color: unref(color) }, null, 8, _hoisted_2$19)])], 512);
		};
	}
});
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				...
				ref: dodecahedronRef
			}, [createElementVNode("TresDodecahedronGeometry", { args: unref(args) }, null, 8, _hoisted_1$22), renderSlot(_ctx.$slots, "default", {}, () => [createElementVNode("TresMeshBasicMaterial", { color: unref(color) }, null, 8, _hoisted_2$18)])], 512);
		};
	}
});
...
//#endregion
//#region src/core/shapes/Grid.vue?vue&type=script&setup=true&lang.ts
const _hoisted_1$21 = [
	"side",
	"cell-size",
	"section-size",
...
var Grid_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
  ...
  `) });
		...
		return (_ctx, _cache) => {
			...
			}, null, 8, _hoisted_1$21), createElementVNode("TresPlaneGeometry", { args: props.args }, null, 8, _hoisted_2$17)], 512);
		};
	}
});

//#endregion
//#region src/core/shapes/Grid.vue
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				...
				ref: icosahedronRef
			}, [createElementVNode("TresIcosahedronGeometry", { args: unref(args) }, null, 8, _hoisted_1$20), renderSlot(_ctx.$slots, "default", {}, () => [createElementVNode("TresMeshBasicMaterial", { color: unref(color) }, null, 8, _hoisted_2$16)])], 512);
		};
	}
});
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				...
				ref: octahedronRef
			}, [createElementVNode("TresOctahedronGeometry", { args: unref(args) }, null, 8, _hoisted_1$19), renderSlot(_ctx.$slots, "default", {}, () => [createElementVNode("TresMeshBasicMaterial", { color: unref(color) }, null, 8, _hoisted_2$15)])], 512);
		};
	}
});
...
var Plane_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			...
			}, [createElementVNode("TresPlaneGeometry", { args: unref(args) }, null, 8, _hoisted_2$14), renderSlot(_ctx.$slots, "default", {}, () => [createElementVNode("TresMeshBasicMaterial", { color: unref(color) }, null, 8, _hoisted_3$4)])], 8, _hoisted_1$18);
		};
	}
});
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				ref_key: "ringRef",
				ref: ringRef
			}, [createElementVNode("TresRingGeometry", { args: unref(args) }, null, 8, _hoisted_1$17), renderSlot(_ctx.$slots, "default", {}, () => [createElementVNode("TresMeshBasicMaterial", { color: unref(color) }, null, 8, _hoisted_2$13)])], 512);
		};
	}
});
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				...
				ref: roundedBoxRef
			}, [createElementVNode("TresRoundedBoxGeometry", { args: unref(args) }, null, 8, _hoisted_1$16), renderSlot(_ctx.$slots, "default", {}, () => [createElementVNode("TresMeshBasicMaterial", { color: unref(color) }, null, 8, _hoisted_2$12)])], 512);
		};
	}
});
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				...
				"frustum-culled": false
			}, [renderSlot(_ctx.$slots, "default")], 8, _hoisted_1$15);
		};
	}
});
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				ref_key: "sphereRef",
				ref: sphereRef
			}, [createElementVNode("TresSphereGeometry", { args: unref(args) }, null, 8, _hoisted_1$14), renderSlot(_ctx.$slots, "default", {}, () => [createElementVNode("TresMeshBasicMaterial", { color: unref(color) }, null, 8, _hoisted_2$11)])], 512);
		};
	}
});
...
var Superformula_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			...
			}, [renderSlot(_ctx.$slots, "default", {}, () => [createElementVNode("TresMeshBasicMaterial", { color: color.value }, null, 8, _hoisted_2$10)])], 8, _hoisted_1$13);
		};
	}
});
...
const _hoisted_1$12 = ["rotation"];
const _hoisted_2$9 = ["args"];
...
var Tetrahedron_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			...
			}, [createElementVNode("TresTetrahedronGeometry", { args: unref(args) }, null, 8, _hoisted_2$9), renderSlot(_ctx.$slots, "default", {}, () => [createElementVNode("TresMeshBasicMaterial", { color: unref(color) }, null, 8, _hoisted_3$3)])], 8, _hoisted_1$12);
		};
	}
});
...
const _hoisted_1$11 = ["args"];
const _hoisted_2$8 = ["color"];
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				ref_key: "torusRef",
				ref: torusRef
			}, [createElementVNode("TresTorusGeometry", { args: unref(args) }, null, 8, _hoisted_1$11), renderSlot(_ctx.$slots, "default", {}, () => [createElementVNode("TresMeshBasicMaterial", { color: unref(color) }, null, 8, _hoisted_2$8)])], 512);
		};
	}
});
...
const _hoisted_1$10 = ["args"];
const _hoisted_2$7 = ["color"];
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				...
				ref: torusKnotRef
			}, [createElementVNode("TresTorusKnotGeometry", { args: unref(args) }, null, 8, _hoisted_1$10), renderSlot(_ctx.$slots, "default", {}, () => [createElementVNode("TresMeshBasicMaterial", { color: unref(color) }, null, 8, _hoisted_2$7)])], 512);
		};
	}
});
...
const _hoisted_1$9 = ["args"];
const _hoisted_2$6 = ["color"];
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				ref_key: "tubeRef",
				ref: tubeRef
			}, [createElementVNode("TresTubeGeometry", { args: unref(args) }, null, 8, _hoisted_1$9), renderSlot(_ctx.$slots, "default", {}, () => [createElementVNode("TresMeshBasicMaterial", { color: unref(color) }, null, 8, _hoisted_2$6)])], 512);
		};
	}
});
...
const _hoisted_1$8 = ["receive-shadow", "rotation"];
const _hoisted_2$5 = ["args"];
var Backdrop_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			...
			}, null, 8, _hoisted_2$5), renderSlot(_ctx.$slots, "default", {}, () => [_cache[0] || (_cache[0] = createElementVNode("TresMeshStandardMaterial", {
				color: 8421504,
				side: 2
			}, null, -1))])], 8, _hoisted_1$8)], 512);
		};
	}
});
...
var component_default$4 = component_vue_vue_type_script_setup_true_lang_default$1;
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresMesh", {
				ref_key: "mesh",
				ref: mesh
			}, [__props.form === "circle" ? (openBlock(), createElementBlock("TresRingGeometry", _hoisted_1$7)) : __props.form === "ring" ? (openBlock(), createElementBlock("TresRingGeometry", _hoisted_2$4)) : __props.form === "rect" ? (openBlock(), createElementBlock("TresPlaneGeometry", _hoisted_3$2)) : (openBlock(), createBlock(props.form, {
				key: 3,
				args: __props.args
...
const _hoisted_1$6 = ["rotation-x", "args"];
var Ocean_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			...
			}, [renderSlot(_ctx.$slots, "default", {}, () => [_cache[0] || (_cache[0] = createElementVNode("TresPlaneGeometry", { args: [1e4, 1e4] }, null, -1))])], 8, _hoisted_1$6);
		};
	}
});
...
const _hoisted_2$3 = ["position", "velocity"];
var Precipitation_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			...
			}, null, 8, _hoisted_1$5), createElementVNode("TresBufferGeometry", {
				ref_key: "geometryRef",
				ref: geometryRef,
				position: [unref(positionArray), 3],
				velocity: [unref(velocityArray)]
			}, null, 8, _hoisted_2$3)], 512);
		};
	}
});
...
const _hoisted_1$3 = ["scale"];
const _hoisted_2$2 = ["position"];
const _hoisted_3$1 = ["position", "scale"];
const _hoisted_4$1 = [
	"map",
	"depth-test",
	"color-space",
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresGroup", {
				...
				return openBlock(), createElementBlock("TresMesh", {
					...
					transparent: "",
					opacity: unref(opacity)
				}, null, 8, _hoisted_4$1)], 8, _hoisted_3$1);
			}), 128))], 8, _hoisted_2$2)], 8, _hoisted_1$3);
		};
	}
});
...
var component_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
    ...
    void main() {
      ...
      #include <${version$1 >= 154 ? "colorspace_fragment" : "encodings_fragment"}>
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("primitive", { object: unref(sparkles) }, null, 8, _hoisted_1$2);
		};
	}
});
...
var component_default$16 = component_vue_vue_type_script_setup_true_lang_default;

//#endregion
//#region src/core/staging/Stage.vue?vue&type=script&setup=true&lang.ts
const _hoisted_1$1 = { key: 0 };
const _hoisted_2$1 = ["intensity"];
const _hoisted_3 = [
	"position",
	"intensity",
	"castShadow",
...
const _hoisted_4 = ["position", "intensity"];
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("TresGroup", {
				...
				props.lighting ? (openBlock(), createElementBlock("TresGroup", _hoisted_1$1, [
					createElementVNode("TresAmbientLight", { intensity: __props.intensity / 3 }, null, 8, _hoisted_2$1),
				...
				createVNode(component_default$2, mergeProps({
					ref_key: "boundsRef",
					ref: boundsRef,
					...
					default: withCtx(() => [createVNode(Align_default, mergeProps({
						ref_key: "alignRef",
						ref: alignRef
				...
				createElementVNode("TresGroup", { position: [
					...
					(openBlock(), createBlock(Suspense, null, {
						default: withCtx(() => [environmentComputed.value ? (openBlock(), createBlock(component_default$4, normalizeProps(mergeProps({ key: 0 }, environmentComputed.value)), null, 16)) : createCommentVNode("v-if", true)]),
						_: 1
					}))
				], 8, _hoisted_5)
			], 512);
...
const _hoisted_1 = ["position", "a-scale"];
const _hoisted_2 = [
	"size",
	"size-attenuation",
	"transparent",
...
var Stars_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			...
			}, null, 8, _hoisted_1), createElementVNode("TresPointsMaterial", {
				size: unref(size),
				"size-attenuation": unref(sizeAttenuation),
				transparent: unref(transparent),
				"alpha-test": unref(alphaTest),
				"alpha-map": unref(alphaMap)
			}, null, 8, _hoisted_2)], 512);
		};
	}
});
